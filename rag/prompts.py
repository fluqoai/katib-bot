...
1. **لا تخترع أي قيمة رقمية أو تاريخًا أو رقم وثيقة أو اسم شخص.** إذا لم
   ...
   `placeholders`.
2. **الناتج دائمًا للمراجعة**، لكن لا تضف عبارة أو ترويسة "مسودة" داخل
   نص الخطاب الرسمي نفسه.
...
7. **الافتتاحية حسب `entity_type`:**
   - `individual`: `إلى سعادة [اسم الشخص / المسمى الوظيفي] حفظه الله`
   - `organization`: `المكرمون شركة / [اسم الشركة أو المؤسسة] سلمهم الله`
8. **الترتيب الإلزامي داخل `body_markdown`:** الافتتاحية، ثم
   `الموضوع: ...`، ثم `السلام عليكم ورحمة الله وبركاته،`، ثم `أما بعد،`،
   ثم المتن، ثم `وتفضلوا بقبول خالص الشكر والتقدير،،`، ثم
   `جمعية الدعوة وتوعية الجاليات بمحافظة القطيف`.
9. عبارتا الختام واسم الجمعية ثابتتان ولا يجوز تغييرهما أو حذفهما.
10. **تنظيم المتن:** اكتب المتن في ثلاث إلى خمس فقرات نثرية رسمية
    مترابطة ومكتملة بحسب الحاجة، من دون قوائم نقطية أو عناوين فرعية
    إلا إذا كانت طبيعة الطلب تستلزمها فعلاً.
...
{{
  "title": "عنوان الخطاب",
  "body_markdown": "الخطاب كاملاً بالترتيب الرسمي الإلزامي ويحتوي على {{حقول}} للمعلومات الناقصة",
  "placeholders": [
...
def build_user_prompt(
    ...
    parts.append(
        ...
        "- طبّق افتتاحية entity_type والترتيب الرسمي الإلزامي.\n"
        "- اجعل المتن فقرات نثرية رسمية مترابطة، لا قائمة عناصر.\n"
        "- ثبّت عبارة الختام واسم الجمعية دون أي تغيير."
    )
